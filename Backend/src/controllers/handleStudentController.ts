...
import {
  addStudentIdToStdSub,
  changeStudentFromOneClassToAnother,
  deleteStudentIdFromStdSub,
} from "./handleSubjectStandardController";
...
export async function editStudentEndpoint(req: any, res: any) {
  ...
  try {
    ...
    if (oldStandardId !== newStandardId) {
        ...
        console.log("Changing student from one class to another");
        

      const response = await changeStudentFromOneClassToAnother(
          studentId,
        oldStandardId,
        newStandardId
      );
      ...
      if (response) {
        res.status(200).send({ message: "Student edited and moved to new class successfully" });
      } else {
        res.status(500).send({ message: "Error changing student!! Please DELETE and ADD again" });
      }
    } else {
